...
\begin{document}
	...
	Music strongly shapes how a run feels and performs, yet a runner's musical needs shift continuously with two things that often disagree: the goal they set out to pursue, and the effort their body is actually producing. Existing systems address at most one side. Early physiological players such as MPTrain~\cite{oliver2006mptrain} adapt music to heart rate, but require the goal as a numeric target rather than in words and fold adaptation and safety into a single opaque model; practical open-source apps~\cite{wu2020heartbpmusic, ray2018heartbeat} merely match song tempo to heart rate, with no notion of stated intent and no safety limit. This is relevant to Artificial Intelligence beyond its sporting framing because the task is essentially one of fusing signals that can conflict, through a decision layer that stays inspectable rather than fully learned end-to-end.

	RUNMAXXIN addresses this gap. It reads a short natural-language prompt to capture the runner's goal and mood, interprets heart rate and cadence to estimate real effort, and fuses both into an acoustic target that a recommender turns into songs, while an explicit symbolic layer guarantees the rules that must always hold -- most importantly steering toward recovery when the heart is overworked. Our contribution is not a new model but a set of deliberate choices that we then test rather than assume. We use a lightweight few-shot text classifier instead of fine-tuning a large transformer, and measure the trade-off against a rule-based baseline. We also show that the statistical recommender, despite computing a probability over candidates, ignores the runner's safety limits in a measurable share of cases, and we correct this with a small resampling step. In both respects the project is a critical analysis and improvement of existing components rather than a system presented as flawless.
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.95\textwidth]{figures/setfit-architecture.png}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.62\textwidth]{figures/setfit-fewshot-cr.png}
